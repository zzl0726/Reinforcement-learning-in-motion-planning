...
import numpy as np
import random
from config import VISION_W, VISION_F, VISION_B, ROUND, DL_IS_TRAINING, BATCH_SIZE, trace, h, w, lstm_units, TEMP
...
class Cnn:
    def __init__(self, model_name, replay_memory, num_actions=5, target=False):
        ...
        with self.graph.as_default():
            ...
            self.q_values = net
            self.q_dis = tf.nn.softmax(self.q_values/TEMP)
    ...
    def get_q_values(self, states, actions):
        ...
        values = self.session.run([self.q_values, self.q_dis], feed_dict=feed_dict)
