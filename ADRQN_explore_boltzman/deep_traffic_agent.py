...
from config import LEARNING_RATE, EPSILON_GREEDY_START_PROB, EPSILON_GREEDY_END_PROB, EPSILON_GREEDY_MAX_STATES, \
    MAX_MEM, BATCH_SIZE, VISION_W, VISION_B, VISION_F, TARGET_NETWORK_UPDATE_FREQUENCY, LEARN_START, trace
...
class DeepTrafficAgent:
    def __init__(self, model_name):
        ...
        self.previous_states = np.zeros([1, VISION_F + VISION_B + 1, VISION_W * 2 + 1, trace])
        self.previous_actions = np.zeros([trace])
    ...
    def act(self, state, is_training=True):
        ...
        self.previous_states = self.previous_states.reshape(1, VISION_F + VISION_B + 1, VISION_W * 2 + 1, trace)
    ...
    def remember(self, reward, next_state, end_episode=False, is_training=True):
        ...
        next_state = np.array(previous_states).reshape(-1, VISION_F + VISION_B + 1, VISION_W * 2 + 1, trace)
        ...
        if is_training and self.model.get_count_states() > LEARN_START and len(self.memory) > LEARN_START:
            ...
            if self.model.get_count_states() % TARGET_NETWORK_UPDATE_FREQUENCY == 0:
                ...
                state_train = (np.zeros([BATCH_SIZE,10]),np.zeros([BATCH_SIZE,10]))
                self.model.state_in = state_train
        ...
        if end_episode:
            ...
            self.score = 0
            # Reset the recurrent layer's hidden state
            # here h_size for lstm is 10
            state = (np.zeros([1,10]),np.zeros([1,10]))
            self.model.state_in = state
...
class LinearControlSignal:
    ...
    def get_value(self, iteration):
        ...
        if iteration < self.num_iterations:
            # linear decay of epsilon greedy
