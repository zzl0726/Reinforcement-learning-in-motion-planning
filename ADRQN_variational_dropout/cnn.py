...
import numpy as np
import random
from config import VISION_W, VISION_F, VISION_B, ROUND, DL_IS_TRAINING, BATCH_SIZE, trace, h, w, lstm_units, DROP_OUT_PROB
...
class Cnn:
    def __init__(self, model_name, replay_memory, num_actions=5, target=False):
        ...
        with self.graph.as_default():
            ...
            # try the tanh activation instead of relu
            activation = tf.nn.tanh
            # activation = tf.nn.leaky_relu # won't help much
            ...
            self.x = tf.placeholder(dtype=tf.float32,
                                    shape=[None, VISION_B + VISION_F + 1, VISION_W * 2 + 1, trace],
                                    name='x')
            ...
            # Add a recurrent layer here
            # Output here should be [None, 288] i.e., 9*1*32
            net = tf.reshape(net, [-1, h*w*32])


            # ActionDense1
            # The action network to implement ADRQN
            # Originally each dense layer had 4 units
            nnet = tf.reshape(self.actions, [-1, trace])
            nnet = tf.layers.dense(inputs=nnet, units=32, activation=activation, kernel_initializer=init)
            nnet = tf.layers.dense(inputs=nnet, units=64, activation=activation, kernel_initializer=init)

            # Merge Conv2 and ActionDense1
            net = tf.concat([net, nnet], 1)
            batch_dim = tf.shape(net)[0]


            # Adding the recurrent layer
            # We fix the batch size for both train and target 
            # For now our trace is the number of time steps

            # self.train_length = tf.placeholder(dtype=tf.int32)
            # self.batchsize = tf.placeholder(dtype=tf.int32,shape=[])

            #We define the cells for the primary and target q-networks
            # variational dropouts inside the lstm layer
            rnn_cell = tf.contrib.rnn.BasicLSTMCell(num_units=lstm_units,state_is_tuple=True)
            rnn_cell = tf.nn.rnn_cell.DropoutWrapper(rnn_cell, output_keep_prob=DROP_OUT_PROB)

            # Input to LSTM must be [batch_size x trace x out_units] 
            # Batch size was always preserved
            self.convFlat = tf.reshape(net,[batch_dim,trace,lstm_units])

            self.state_in = rnn_cell.zero_state(batch_dim, tf.float32)

            self.rnn,self.rnn_state = tf.nn.dynamic_rnn(inputs=self.convFlat,cell=rnn_cell,dtype=tf.float32,
                                                        initial_state=self.state_in)

            self.rnnout = tf.reshape(self.rnn[:, tf.shape(self.rnn)[1]-1, :], shape=[-1,lstm_units])

            # net = tf.nn.dropout(self.rnnout, DROP_OUT_PROB)

            # net = tf.layers.dense(inputs = self.rnn, units=10, activation=activation, kernel_initializer=init)
            net = tf.layers.dense(inputs = self.rnnout, units=5, activation=None, kernel_initializer=init)

            self.q_values = net
            # print(self.q_values.shape)
            ...
            # Look how to implement this??
            # self.maskA = tf.zeros([BATCH_SIZE,trace//2])
            # self.maskB = tf.ones([BATCH_SIZE,trace//2])
            # self.mask = tf.concat([self.maskA,self.maskB],1)
            # self.mask = tf.reshape(self.mask,[-1])
            # self.loss = tf.reduce_mean(sum_squared_error * self.mask)
    ...
    def get_memory_component(self, memory, batch_size, target_network=None):
        ...
        states = np.array(states).reshape(-1, VISION_B + VISION_F + 1, VISION_W * 2 + 1, trace)
